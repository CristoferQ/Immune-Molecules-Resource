...
data_type = sys.argv[3]#Type of input (json/fasta)
database = sys.argv[2]#fasta or json file sequence
dict_responses = {}
if(data_type == "fasta"):
	fasta_check, fasta_len= (is_fasta(database))
			...
			if fasta_len <=50:
				...
				for record in SeqIO.parse(database, "fasta"):				
					try:
						...
						dict_response_array.append({"id_sequence":record.id, "status_service": "ERR", "Sequence": sequence})
...
if(data_type == "json"):
	...
	if len(string_map)>=5 and len(string_map)<=30:
		...
		if length <=50:
			dict_responses.update({"length_status":"OK"})
			dict_response_array = []
			for record in readJson(database):
				try:
					id_sequence = record["id_sequence"]
					sequence = [residue for residue in record["Sequence"]]
					sequence = ''.join(map(str, sequence))
					dict_response_search = {"id_sequence":id_sequence,"Sequence": sequence }
					#search using method
					dict_response_search.update({"response_search":mapping_string(string_map, sequence)})
					...
					dict_response_array.append({"id_sequence":record["id_sequence"], "status_service": "ERR","Sequence": sequence})
			dict_responses.update({"mapping_service":dict_response_array})
		else:
			dict_responses.update({"length_status":"ERR"})
	else:

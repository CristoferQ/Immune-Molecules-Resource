...
$("#submit").on("click", function(){
    ...
    $.ajax({
        url: "http://localhost:5001/uploadFile",
        method: "POST",
        data: formData,
        dataType: "html",
        contentType: false,
...
function go_ajax(name, go){
    if(go){
        return $.ajax({
            url: "http://localhost:5001/ServiceGeneOntology/" + name,
            method: "GET"
        })
    }
}
function pfam_ajax(name, pfam){
    if(pfam){
        return $.ajax({
            url: "http://localhost:5001/ServicePFam/" + name,
            method: "GET"
        })
    }
}
function secondary_ajax(name, secondary){
    if(secondary){
        return $.ajax({
            url: "http://localhost:5001/ServiceStructural/" + name,
            method: "GET"
        })
    }
}
function fillData(name, go_data, pfam_data, secondary_data){
    $.ajax({
        url: "http://localhost:5001/getFastaInfo/" + name,
        method: "GET"
    }).done(function(res){
        let matrix = []
